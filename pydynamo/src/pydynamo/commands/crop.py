...
from ..io import read_dynamo_tbl, read_vll_to_df
...
def run(config_path: str, rest: list, args) -> int:
    ...
    if not particles_in or not sidelength or not output_star:
        _err("Missing required: particles, sidelength, output_star", args)
        return 1

    if sidelength <= 0 or sidelength % 2 != 0:
        _err("sidelength must be positive and even", args)
        return 1
        ...
        if particles_in.endswith(".star"):
            ...
            if "rlnCoordinateX" in df.columns:
                ...
                if not pixel_size or not tomogram_size:
                    _err("star with rlnCenteredCoordinate requires pixel_size and tomogram_size", args)
                ts = tuple(tomogram_size)
                df = _star_centered_to_absolute(df, pixel_size, ts)
                x_col, y_col, z_col = "x", "y", "z"
            else:
                _err("Star file must have rlnCoordinateX/Y/Z or rlnCenteredCoordinateXAngst/Y/Z", args)
            tomo_col = "rlnMicrographName" if "rlnMicrographName" in df.columns else "rlnTomoName"
            if tomo_col not in df.columns:
                _err(f"Star file must have {tomo_col}", args)
        else:
            if not vll_path and tomograms is None:
                _err("tbl requires vll or tomograms for tomogram paths", args)
    ...
    if not out_rows:
        _err("No particles were cropped", args)
        return 1
